...
class CUTEstFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, problem):
        global DIR_EVAL_COUNTER
        DIR_EVAL_COUNTER = DIR_EVAL_COUNTER + 1
        global CONVTIME1,CONVTIME2

        ct1 = time.time()

        # x = input.clone().detach().cpu().numpy()
        x = input.cpu()

        ct0 = time.time()
        obj, grad = problem.obj(x, gradient=True)
        CONVTIME1 = CONVTIME1 + time.time() - ct0


        ct0 = time.time()
        ctx.save_for_backward(torch.tensor(grad, dtype=torch.double, device=DEVICE))
        ret = torch.tensor(obj, dtype=torch.double, device=DEVICE)
        # time.sleep(0.02)
        CONVTIME2 = CONVTIME2 + time.time() - ct1

        # print('conv:', CONVTIME1)
        # print('conv:', CONVTIME2)
...
class CUTEstProblem(torch.nn.Module):
    ...
    def __init__(self, problem):
        ...
        # get initialization
        x = torch.tensor(problem.x0, dtype=torch.double, device=DEVICE)
        x.requires_grad_()
