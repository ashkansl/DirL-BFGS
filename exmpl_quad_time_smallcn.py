...
from newfast import NewLBFGS as FAST
...
# w_name = "w100_100d_100s_1000cn.pt"
# y_name = "y100_100d_100s_1000cn.pt"
#--------------------------------------- 
...
def two_f():
    ...
    f.write(f'avg time 1e-8: {np.average(time_iter_sl[:,0])}\n')
    f.write(f'std time 1e-8: {np.std(time_iter_sl[:,0])}\n')
    f.write(f'avg iter 1e-8: {np.average(time_iter_sl[:,1])}\n')
    f.write(f'std iter 1e-8: {np.std(time_iter_sl[:,1])}\n\n')
    f.write(f'avg time min loss: {np.std(time_iter_sl[:,1])}\n\n')
    f.write(f'std time min loss: {np.std(time_iter_sl[:,1])}\n\n')
    ...
    f.write(f'avg time 1e-8: {np.average(time_iter_sl[:,4])}\n')
    f.write(f'std time 1e-8: {np.std(time_iter_sl[:,4])}\n')
    f.write(f'avg iter 1e-8: {np.average(time_iter_sl[:,5])}\n')
    f.write(f'std iter 1e-8: {np.std(time_iter_sl[:,5])}\n\n')
    ...
    f.write(f'avg time 1e-8: {np.average(time_iter_sl[:,2])}\n')
    f.write(f'std time 1e-8: {np.std(time_iter_sl[:,2])}\n')
    f.write(f'avg iter 1e-8: {np.average(time_iter_sl[:,3])}\n')
    f.write(f'std iter 1e-8: {np.std(time_iter_sl[:,3])}\n\n')
...
def plot_loss_time_glob_way_quad(plot_time=True, name_filter='.'):
    # name_filter = '.pt'
    path = './utils_input/*.pt'
    file_list = []
    for fn in glob(path):
        # if str(fn).find(name_filter) > 0:
        if str(fn).find(name_filter) > -1:  
            file_list.append(fn)
    ...
    for item in file_list:
        if item.find('hist') > 0:
            history_lbfgs.append(torch.load(str(item)))
            names.append(item)
        if item.find('time') > 0:
            if plot_time:
                time_list.append(torch.load(str(item)))
            else:
                tempload = torch.load(str(item))
                tlist = []
                for ti in range(len(tempload)):
                    tlist.append(ti)
    ...
    k = 0
    st = 0
    for h,t,name in zip(history_lbfgs, time_list, names):
        hc = []
        for it1 in h:
            # hc.append(it1.detach().cpu().numpy())
            hc.append(it1)
            
        
        l1 = name.replace('\\', '/').split('/')[-1]
        nn = l1.split('_')[0] + ' ' + l1.split('_')[-1].split('.')[0]
        minus = t[st]
        if l1.find('bfgs')>-1 and  l1.find('lbfgs')<0:
            
            axs.semilogy(t[st:], hc[st:], label=nn, c='k', linestyle='-.', linewidth=1.0)
            i = i + 1
        if l1.find('lbfgs')>-1:
            axs.semilogy(t[st:], hc[st:], label=nn, c=colors[j], linestyle='--', linewidth=1.0) # , marker='.',markevery=10)
            j = j + 1
        if l1.find('new')>-1:
            axs.semilogy(t[st:], hc[st:], label=nn, c=colors[k], linestyle='-', linewidth=1.0) #, marker='*',markevery=12)
            k = k + 1
            
 
    if plot_time:
        plt.xlim(-0.02, 0.5)
        save_name = '_avg_time_plot.png'
    else:
        plt.xlim(-5, 150)
        save_name = '_avg_iter_plot.png'
...
if __name__ == "__main__":
    ...
    two_f()
    # plot_loss_time_glob_way_quad(plot_time=False, name_filter='.') # for quad pts
